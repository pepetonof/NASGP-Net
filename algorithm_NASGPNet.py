...
def eaNASGPNet(pop_size, toolbox, cxpb, mutpb, ngen, nelit,
    ...
    if checkpoint_name:
        ...
        print('Idx Pop: \t', idx)
        print('Best', str(tools.selBest(population, 1)[0]), round(tools.selBest(population, 1)[0].fitness.values[0],3), tools.selBest(population, 1)[0].params)
        ...
        while idx < len(invalid_ind):
            ...
            if key in cache:
                ...
                if start_gen==0 and idx==0:
                    fit = toolbox.evaluate(ind, save_data=True)
                else:
                    fit = toolbox.evaluate(ind)
                # #Assign attributes from the original objective function
                # if surrogate == None:
                # fit = toolbox.evaluate(ind, save_data=f)
