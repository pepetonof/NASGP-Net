# -*- coding: utf-8 -*-
"""
Created on Tue Jun  6 18:22:22 2023
...
Surrogate Algorithmm based on
A Global and Local Surrogate-Assisted Genetic
Programming Approach to Image Classification
Qinglan Fan, Graduate Student Member, IEEE, Ying Bi, Member, IEEE,
...
def MateMutationParentFitness(population, toolbox, lambda_, cxpb, mutpb):
    assert (cxpb + mutpb) <= 1.0, (
        "The sum of the crossover and mutation probabilities must be smaller "
        "or equal to 1.0.")
    mut_options = ['shrink','replace','uniform']
    ...
    for _ in range(lambda_):
        ...
        if op_choice < cxpb:            # Apply crossover
            ...
            ind1, ind2 = list(map(toolbox.clone, sample))
            
            dice_p1=ind1.dice
            dice_p2=ind2.dice
            dice_m = np.mean(np.array([dice_p1,dice_p2]))
            
            ind1, ind2 = toolbox.mate(ind1, ind2)
            
            ind1.dice_p1 = dice_p1
            ind1.dice_p2 = dice_p2
            ind1.dice_m  = dice_m
...
#%%NASGP-Net Algorithm
def eaNASGP_Net2(pop_size, toolbox, cxpb, mutpb, ngen, nelit,
                gen_update, p, m,
    ...
    if checkpoint_name:
        ...
        # train_pop    = cp["train_pop"]
        train_set    = cp["train_set"]
        train_labels = cp["train_labels"]
        surr_metrics = cp["surr_metrics"]
        rf           = cp["rf"]
        no_evs       = cp["no_evs"]
        delta_t      = cp["delta_t"]
        cache        = cp["cache"]
    ...
    if idx==len(invalid_ind) or checkpoint_name==False:
        ...
        for gen in range(start_gen, ngen+1):
            ...
            logbook.record(gen=gen, nevals=no_evs, time=delta_t,
                           ...
                           **record)
        print("Time", delta_t)
        return population, logbook, archive, cache


#%%NASGP-Net Algorithm
def eaNASGP_Net(pop_size, toolbox, cxpb, mutpb, ngen, nelit,
                gen_update, p, m,
                ruta, checkpoint_name,
                stats=None, halloffame=None, verbose_evo=__debug__,
                ):
